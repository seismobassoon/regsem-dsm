module read_model !!! PREM + Random variations !!!
...
doubleprecision, parameter :: Rterre = 6371000, mille = 1000.d0
...
call get_variations{theta_rad,phi_rad,dvp,dvs}
vp = vp*(1.d0+dvp)   ! OR vp = vp*(1.d0+dvp/100.d0)
vs = vs*(1.d0+dvs)   ! OR vs = vs*(1.d0+dvs/100.d0)
...
print *, "NO ANISOTROPY IN THE RANDOM MODEL SO FAR"
stop

end subroutine get_value_aniso

! #######################################################
subroutine get_variations(theta,phi,dvp,dvs)

implicit none

doubleprecision, intent(IN) :: theta,phi   !!! Les coordonnees d'entree sont en radian !!!
doubleprecision, intent(INOUT) :: dvp,dvs

dvp = 0.d0
dvs 0.d0

end subroutine get_variations
