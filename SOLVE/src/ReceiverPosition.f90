...
do n_rcp = 0, Tdomain%n_receivers-1
 ...
 if (Tdomain%curve) then
    ...
    if (Tdomain%ellipticity) then
        R = Rterre * (1.d0 - get_ellipticity(Rterre)*(3.d0*dcos(pi*colat/180)**2.d0 - 1.d0)/3.d0)
    else
        R = Rterre
    endif
    ...
    if (Tdomain%comp_rot) then   ! Calcul de l'angle de passage du systeme Z/N/E au systeme Z/R/T
       ...
       if (Tdomain%sReceiver(n_rcp)%cosgamma<1) then
          Tdomain%sReceiver(n_rcp)%singamma = dsqrt(1-Tdomain%sReceiver(n_rcp)%cosgamma**2)
       else
          Tdomain%sReceiver(n_rcp)%singamma = 0
       endif
    endif
 else
 ...
 if (rg==Tdomain%sReceiver(n_rcp)%proc) then
    ...
    enddo
    dmin = max (dsqrt((coord(0,0)-coord(6,0))**2 + (coord(0,1)-coord(6,1))**2 + (coord(0,2)-coord(6,2))**2), &
                dsqrt((coord(1,0)-coord(7,0))**2 + (coord(1,1)-coord(7,1))**2 + (coord(1,2)-coord(7,2))**2), &
                dsqrt((coord(2,0)-coord(4,0))**2 + (coord(2,1)-coord(4,1))**2 + (coord(2,2)-coord(4,2))**2), &
                dsqrt((coord(3,0)-coord(5,0))**2 + (coord(3,1)-coord(5,1))**2 + (coord(3,2)-coord(5,2))**2))
    epsil = dmin/10000.
    num = 0
