...
integer :: length, i, j, npml, mat, ok, nf, ne


! Read simulation features
open (11,file="input.spec",form="formatted",status="old")
read (11,*) Tdomain%sTimeParam%duration
read (11,*) Tdomain%sTimeParam%alpha
read (11,*) Tdomain%sTimeParam%beta
read (11,*) Tdomain%sTimeParam%gamma
...
read (11,*) Tdomain%save_snapshots
if (Tdomain%save_snapshots) then
...
do i = 0, Tdomain%n_elem - 1
   read(12,*) Tdomain%specel(i)%mat_index, Tdomain%specel(i)%moho_position, &
              Tdomain%specel(i)%topo_position, Tdomain%specel(i)%mirror_position
enddo
...
if (Tdomain%special_z_crust/=0) then
    ...
    do i = 0, Tdomain%n_elem-1
        ...
        if (mat==6 .or. &
            ...
            print *,"YOU CAN'T USE A SPECIAL NGLLZ WHEN YOU HAVE PML AT THE SURFACE"
            stop
        endif
