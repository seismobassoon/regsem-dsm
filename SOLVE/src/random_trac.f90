...
doubleprecision, parameter :: epsil = 0.000000001   ! Le zero des reels
doubleprecision :: dt = 1.0d0   ! Pas de temps des signaux aleatoires
taper_debut = Tdomain%sSource(isrc)%tau_b   ! Longueur du signal qui sera attenue au debut
taper_fin = 0.d0   ! Longueur du signal qui sera attenue a la fin
      ...
      if (Tdomain%specel(elem)%random_t) then
         ...
         do y = 1,nglly-2
          do x = 1,ngllx-2
             ...
             call comp_rand_t (Amp, epsil, dt, taper_debut, taper_fin, duree, nstep2, spectre, nstep, Ffilt)
         ...
         do y = 0,nglly-1
          do x = 0,ngllx-1
             ...
             Tdomain%specel(n)%random_coeff(x,y,:,:) = Tdomain%specel(n)%random_coeff(x,y,:,:) * &
                                                       ...
                                                       sqrt(normale(0)**2 + normale(1)**2 + normale(2)**2) * &
