...
do k = 0,1
  ...
  if (k==0 .and. u(0)==0) then
      Tdomain%specel(n)%Normales(0,k) = 1
      Tdomain%specel(n)%Normales(1,k) = 0
      Tdomain%specel(n)%Normales(2,k) = 0
      !!! (u(1)==0 .and. v(1)==0) indicates that face 2 or 4 is in the (x,z) plan.
      !!! It is impossible in the cases we considere.
      !!! (v(0)==0 .and. v(1)==0) indicates that v==z.
      !!! It is impossible if Xi <= 90
  else if (k==1 .and. u(1)==0 .and. v(1)==0) then
      Tdomain%specel(n)%Normales(0,k) = 0
      Tdomain%specel(n)%Normales(1,k) = 1
      Tdomain%specel(n)%Normales(2,k) = 0
      ...
      Tdomain%specel(n)%Normales(2,k) = -1. / sqrt((a**2+1)*c**2 + 2*a*b*c + b**2 + 1.)
