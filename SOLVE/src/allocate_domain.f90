...
do n = 0,Tdomain%n_elem-1
    ...
    do i = 0,Tdomain%nb_simu-1
        ...
        Tdomain%specel(n)%sSimu(i)%Veloc = 0   
        Tdomain%specel(n)%sSimu(i)%Accel = 0
    enddo
    if (Tdomain%specel(n)%PML) then
        ...
        do i = 0,Tdomain%nb_simu-1
            ...
            Tdomain%specel(n)%sSimu(i)%Diagonal_Stress = 0
            Tdomain%specel(n)%sSimu(i)%Diagonal_Stress1 = 0
            Tdomain%specel(n)%sSimu(i)%Diagonal_Stress2 = 0
            Tdomain%specel(n)%sSimu(i)%Diagonal_Stress3 = 0
            Tdomain%specel(n)%sSimu(i)%Residual_Stress = 0
            Tdomain%specel(n)%sSimu(i)%Residual_Stress1 = 0
            Tdomain%specel(n)%sSimu(i)%Residual_Stress2 = 0
            Tdomain%specel(n)%sSimu(i)%Veloc1 = 0
            Tdomain%specel(n)%sSimu(i)%Veloc2 = 0
            Tdomain%specel(n)%sSimu(i)%Veloc3 = 0
        enddo
        if (Tdomain%curve) then
            ...
            do i = 0,Tdomain%nb_simu-1
                ...
                Tdomain%specel(n)%sSimu(i)%Residual_Stress3 = 0
            enddo
        endif
    else
...
!        allocate (Tdomain%specel(n)%Acoeff (0:ngllx-1, 0:nglly-1, 0:ngllz-1, 0:44))
        do i = 0,Tdomain%nb_simu-1
            ...
            Tdomain%specel(n)%sSimu(i)%Displ = 0
            Tdomain%specel(n)%sSimu(i)%V0 = 0
        enddo
        if (Tdomain%aniso) then
            ...
            if (Tdomain%adjoint) then
                ...
                Tdomain%specel(n)%Kern_rho = 0
                Tdomain%specel(n)%Kern_aniso = 0
                do i = 0,Tdomain%nb_simu-1
            ...
            if (Tdomain%adjoint) then
                ...
                Tdomain%specel(n)%Kern_rho = 0
                Tdomain%specel(n)%Kern_lambda = 0
                Tdomain%specel(n)%Kern_mu = 0
                do i = 0,Tdomain%nb_simu-1
                    allocate (Tdomain%specel(n)%sSimu(i)%save_strain (0:ngllx-1, 0:nglly-1, 0:ngllz-1, 0:5))
                enddo
                allocate (Tdomain%specel(n)%anomaly (0:ngllx-1, 0:nglly-1, 0:ngllz-1))
        ...
        if (n_solid>0) then
            ...
            do i = 0,Tdomain%nb_simu-1
                ...
                Tdomain%specel(n)%sSimu(i)%epsilondev_xx_ = 0
                Tdomain%specel(n)%sSimu(i)%epsilondev_yy_ = 0
                Tdomain%specel(n)%sSimu(i)%epsilondev_xy_ = 0
                Tdomain%specel(n)%sSimu(i)%epsilondev_xz_ = 0
                Tdomain%specel(n)%sSimu(i)%epsilondev_yz_ = 0
                Tdomain%specel(n)%sSimu(i)%R_xx_ = 0
                Tdomain%specel(n)%sSimu(i)%R_yy_ = 0
                Tdomain%specel(n)%sSimu(i)%R_xy_ = 0
                Tdomain%specel(n)%sSimu(i)%R_xz_ = 0
                Tdomain%specel(n)%sSimu(i)%R_yz_ = 0
            enddo
        endif
    endif
enddo
...
do n = 0,Tdomain%n_face-1
    ...
    do i = 0,Tdomain%nb_simu-1
        ...
        Tdomain%sFace(n)%sSimu(i)%Veloc = 0   
        Tdomain%sFace(n)%sSimu(i)%Accel = 0  
        Tdomain%sFace(n)%sSimu(i)%Forces = 0
    enddo
    if (Tdomain%sFace(n)%ocean) then
        allocate (Tdomain%sFace(n)%Mocean (1:ngll1-2, 1:ngll2-2))
        Tdomain%sFace(n)%Mocean = 0
    ...
    if (Tdomain%sFace(n)%PML) then 
        ...
        do i = 0,Tdomain%nb_simu-1
            ...
            Tdomain%sFace(n)%sSimu(i)%Veloc1 = 0
            Tdomain%sFace(n)%sSimu(i)%Veloc2 = 0
            Tdomain%sFace(n)%sSimu(i)%Veloc3 = 0
        enddo
    else
        do i = 0,Tdomain%nb_simu-1
            ...
            Tdomain%sFace(n)%sSimu(i)%Displ = 0
            Tdomain%sFace(n)%sSimu(i)%V0 = 0
        enddo
    endif
enddo
...
do n = 0,Tdomain%n_edge-1
    ...
    do i = 0,Tdomain%nb_simu-1
        ...
        Tdomain%sEdge(n)%sSimu(i)%Veloc = 0
        Tdomain%sEdge(n)%sSimu(i)%Accel = 0
        Tdomain%sEdge(n)%sSimu(i)%Forces = 0
    enddo
    if (Tdomain%sEdge(n)%ocean) then
        allocate (Tdomain%sEdge(n)%Mocean (1:ngll-2))
        Tdomain%sEdge(n)%Mocean = 0
    ...
    if (Tdomain%sEdge(n)%PML) then
        ...
        do i = 0,Tdomain%nb_simu-1
            ...
            Tdomain%sEdge(n)%sSimu(i)%Veloc1 = 0
            Tdomain%sEdge(n)%sSimu(i)%Veloc2 = 0
            Tdomain%sEdge(n)%sSimu(i)%Veloc3 = 0
        enddo
    else
        do i = 0,Tdomain%nb_simu-1
            allocate (Tdomain%sEdge(n)%sSimu(i)%Displ (1:ngll-2, 0:2))
            allocate (Tdomain%sEdge(n)%sSimu(i)%V0 (1:ngll-2, 0:2))
            Tdomain%sEdge(n)%sSimu(i)%Displ = 0
            Tdomain%sEdge(n)%sSimu(i)%V0 = 0
        enddo
    endif
enddo
...
do n = 0,Tdomain%n_vertex-1
    ...
    do i = 0,Tdomain%nb_simu-1
        ...
        Tdomain%sVertex(n)%sSimu(i)%Veloc = 0
        Tdomain%sVertex(n)%sSimu(i)%Accel = 0
        Tdomain%sVertex(n)%sSimu(i)%Forces = 0
    enddo
    if (Tdomain%sVertex(n)%ocean) then
        Tdomain%sVertex(n)%Mocean = 0
    ...
    if (Tdomain%sVertex(n)%PML) then
        ...
        do i = 0,Tdomain%nb_simu-1
            ...
            Tdomain%sVertex(n)%sSimu(i)%Veloc1 = 0
            Tdomain%sVertex(n)%sSimu(i)%Veloc2 = 0
            Tdomain%sVertex(n)%sSimu(i)%Veloc3 = 0
        enddo
    else
        do i = 0,Tdomain%nb_simu-1
            allocate (Tdomain%sVertex(n)%sSimu(i)%Displ (0:2))
            allocate (Tdomain%sVertex(n)%sSimu(i)%V0 (0:2))
            Tdomain%sVertex(n)%sSimu(i)%Displ = 0
            Tdomain%sVertex(n)%sSimu(i)%V0 = 0
        enddo
    endif
enddo
